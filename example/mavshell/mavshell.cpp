...
int main(int argc, char** argv)
{
    ...
    if (!start_discovery(dc, connection_url)) {
        return 1;
    }

    wait_until_discover(dc);
    run_interactive_shell(dc, 1000);
...
bool start_discovery(Mavsdk& dc, const std::string& connection_url)
{
...
void run_interactive_shell(Mavsdk& dc, const uint16_t timeout_ms)
{
    Shell shell(dc.system());
